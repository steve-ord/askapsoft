...
void loadLayout(const std::string &fname, casacore::Vector<double> &x, casacore::Vector<double> &y, casacore::Vector<double> &z)
{
  ...
  casacore::Vector<double> xBuf(nAnt);
  casacore::Vector<double> yBuf(nAnt);
  casacore::Vector<double> zBuf(nAnt);
  ...
  if((coordinates == "local") || (coordinates == "longlat") ) {
     const casacore::MPosition location = asMPosition(antParset.getStringVector("location"));
      
     casacore::MVAngle mvLong = location.getAngle().getValue()(0);
     casacore::MVAngle mvLat = location.getAngle().getValue()(1);

     ASKAPLOG_INFO_STR(logger, "Using local coordinates for the antennas: Reference position = "
                              << mvLong.string(casacore::MVAngle::ANGLE, 7) << " "
                              << mvLat.string(casacore::MVAngle::DIG2, 7));
...
/// @param[out] y coordinate Y
/// @param[out] z coordinate Z
void getBaselines(const std::string &fname, casacore::Vector<double> &x, casacore::Vector<double> &y, casacore::Vector<double> &z)
{
  casacore::Vector<double> xAnt,yAnt,zAnt;
  ...
  for (casacore::uInt ant=0; ant<xAnt.nelements(); ++ant) {
       std::cout<<std::scientific<<std::setprecision(15)<<xAnt[ant]<<" "<<yAnt[ant]<<" "<<zAnt[ant]<<std::endl;
  }
  throw 1;
  //
  */         
  const casacore::uInt nBaselines = (xAnt.nelements()*(xAnt.nelements()-1)) / 2;
  x.resize(nBaselines);
  y.resize(nBaselines);
  z.resize(nBaselines);
  for (casacore::uInt ant1=0,baseline=0;ant1<xAnt.nelements(); ++ant1) {
       for (casacore::uInt ant2=0;ant2<ant1; ++ant2,++baseline) {
...
void calculateUVW(const casacore::Vector<double> &x, const casacore::Vector<double> &y, const casacore::Vector<double> &z,
                  double dec, double H0, casacore::Vector<double> &u, casacore::Vector<double> &v,casacore::Vector<double> &w)
{
   ASKAPDEBUGASSERT(x.nelements() == y.nelements());
   ASKAPDEBUGASSERT(x.nelements() == z.nelements());
   ASKAPDEBUGASSERT(y.nelements() == z.nelements());
   const casacore::uInt size = x.nelements();
...
casacore::Vector<double> analyseBaselines(casacore::Vector<double> &x, casacore::Vector<double> &y, casacore::Vector<double> &z)
{
  casacore::Matrix<double> normalMatr(3,3,0.);
  const casacore::uInt nBaselines = x.nelements();
  for (casacore::uInt b = 0; b<nBaselines; ++b) {
       normalMatr(0,0) += casacore::square(x[b]);
       normalMatr(1,1) += casacore::square(y[b]);
       normalMatr(2,2) += casacore::square(z[b]);
  ...
  casacore::Vector<double> normalVector(eVect.column(2).copy());
  const double norm = casacore::sum(casacore::square(normalVector));
  normalVector /= norm;
...
double analyseUVW(casacore::Vector<double> &u, casacore::Vector<double> &v, casacore::Vector<double> &w, 
                  casacore::Vector<double> &nv, bool verbose = false)
{
  nv.resize(3);
  casacore::Matrix<double> normalMatr(3,3,0.);
  const casacore::uInt nBaselines = u.nelements();
  for (casacore::uInt b = 0; b<nBaselines; ++b) {
       normalMatr(0,0) += casacore::square(u[b]);
       normalMatr(1,1) += casacore::square(v[b]);
       normalMatr(2,2) += casacore::square(w[b]);
...
void mapResidualWTerm(const std::string &name, const double longitude, 
            const casacore::Vector<double> &x, const casacore::Vector<double> &y, const casacore::Vector<double> &z)
{
  // sizes
  const int xSize = 512;
  const int ySize = 512;
  // start/stop values
  const double startDec = -casacore::C::pi/2.;
  const double stopDec = 0.;
  const double incH = 2.*casacore::C::pi/double(xSize)*2./3.;
  const double incDec = -(stopDec - startDec)/double(ySize);
  
  // buffer
  casacore::Matrix<float> buf(xSize,ySize,0.);

  // coordinate system
  casacore::CoordinateSystem coords;
  casacore::Matrix<double> xform(2,2,0.);
  xform.diagonal() = 1.;
  const casacore::DirectionCoordinate dc(casacore::MDirection::HADEC, 
                  casacore::Projection(casacore::Projection::STG), casacore::C::pi+incH , startDec,
                  incH,incDec,xform,xSize/2,1.);  
  coords.addCoordinate(dc);
  // filling the buffer
  casacore::Vector<casacore::Double> pixel(2,0.);
  casacore::MVDirection world;
  casacore::Vector<double> u,v,w;
  casacore::Vector<double> unused;
  ...
  casacore::PagedImage<casacore::Float> result(casacore::TiledShape(buf.shape()), coords, name);
  casacore::ArrayLattice<casacore::Float> lattice(buf);
...
/// @param[in] y baseline coordinate Y
/// @param[in] z baseline coordinate Z
void testUVWRotation(const casacore::Vector<double> &x, const casacore::Vector<double> &y, const casacore::Vector<double> &z)
{
  ...
  const double raOffset = 0./180.*casacore::C::pi;
  const double decOffset = 2./180.*casacore::C::pi;
  casacore::MDirection offsetDir(tangentDir);
  offsetDir.shift(-raOffset,decOffset,casacore::True);
  casacore::Vector<double> uOffset,vOffset,wOffset;
  // uvw at longitude where the region is close to transit
  calculateUVW(x,y,z,offsetDir.getValue().getLat(),casacore::C::pi-offsetDir.getValue().getLong(),uOffset,vOffset,wOffset);
  casacore::Vector<double> uTangent,vTangent,wTangent;
  calculateUVW(x,y,z,tangentDir.getValue().getLat(),casacore::C::pi-tangentDir.getValue().getLong(),uTangent,vTangent,wTangent);
  casacore::UVWMachine machine(offsetDir,tangentDir,false, true);
  //casacore::UVWMachine machine(tangentDir,offsetDir,false, true);
  for (casacore::uInt row=0; row<x.nelements(); ++row) {
       casacore::Vector<double> buf(3);
       buf[0]=uOffset[row];
       buf[1]=vOffset[row];
       buf[2]=wOffset[row];
       double delay;
  ...
  casacore::Vector<double> nvTangent, nvOffset;
  ...
  ASKAPLOG_INFO_STR(logger,"is offset "<< raOffset/casacore::C::pi*180.<<" deg in ra and "<<decOffset/casacore::C::pi*180.<<
            " deg in dec and rotated back to the original direction, is "<< acos(dotProduct)/casacore::C::pi*180.<<" deg");
}
...
int main(int argc, char **argv) {
  try {
     ...
     casacore::Vector<double> x,y,z;
     getBaselines(cfgName,x,y,z);
     casacore::Vector<double> normalVector = analyseBaselines(x,y,z);
     ...
     ASKAPLOG_INFO_STR(logger, "Centre of the layout is at longitude "<<centreLong*180/casacore::C::pi<<
                       " and latitude "<<centreLat*180/casacore::C::pi);
