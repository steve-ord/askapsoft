                   ...
                   scimath::DesignMatrix &dm, casacore::Vector<casacore::Double> &residual) const
{
  ...
  vector<casacore::AutoDiff<double> > visDerivBufferSinglePol(2*freq.nelements(),
                          casacore::AutoDiff<double>(0.,nParameters));
                          
  vector<vector<casacore::AutoDiff<double> > > visDerivBuffer(nPol);
  ...
  for (casacore::uInt row=0,offset=0; row<uvw.nelements(); ++row) {
       ...
       for (casacore::uInt pol = 0; pol<nPol; ++pol) {                
            visDerivBuffer[pol] = vector<casacore::AutoDiff<double> >(2*freq.nelements()*nPol,
                          casacore::AutoDiff<double>(0.,nParameters));
       }
