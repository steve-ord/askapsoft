...
public:
   void setUp() {
      ...
      for (int row=0;row<int(itsBuffer.nrow());++row) {
           for (int col=0;col<int(itsBuffer.ncolumn());++col) {
                ...
                itsBuffer(row,col) = casacore::Complex(5.*exp(-(x*x+y*y)/2),0.);
           }
      }
   }
   ...
   void testPeakFind() {
     ...
     CPPUNIT_ASSERT(casacore::abs(ss.peakVal()-5.)<1e-7);
   }
   ...
   void testSupportSearchFloat() {
      casacore::Matrix<casacore::Float> floatBuffer = amplitude(itsBuffer);
      const double cutoff = 1e-2;
      SupportSearcher ss(cutoff);
...
protected:
   ...
   void doSupportSearcherTest(SupportSearcher &ss, const double factor = 1.) {
      ...
      CPPUNIT_ASSERT(casacore::abs(double(ss.support())-2.*expectedHalfWidth)<1.);
      CPPUNIT_ASSERT(casacore::abs(double(ss.symmetricalSupport(itsBuffer.shape()))-
                2.*(expectedHalfWidth+double(casacore::max(itsXOffset,itsYOffset))))<1.);
      CPPUNIT_ASSERT(casacore::abs(double(ss.blc()(0))-(double(itsBuffer.shape()(0))/2+
            itsXOffset-expectedHalfWidth))<1.);
      CPPUNIT_ASSERT(casacore::abs(double(ss.blc()(1))-(double(itsBuffer.shape()(1))/2+
            itsYOffset-expectedHalfWidth))<1.);
      CPPUNIT_ASSERT(casacore::abs(double(ss.trc()(0))-(double(itsBuffer.shape()(0))/2+
            itsXOffset+expectedHalfWidth))<1.);
      CPPUNIT_ASSERT(casacore::abs(double(ss.trc()(1))-(double(itsBuffer.shape()(1))/2+
            itsYOffset+expectedHalfWidth))<1.);
   }
   
private:
   /// @brief a buffer for the test array
   casacore::Matrix<casacore::Complex> itsBuffer;
