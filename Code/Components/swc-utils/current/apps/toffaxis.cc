...
double analyseUVW(const IConstDataAccessor& acc, casacore::Vector<double> &nv, int beam = -1)
{
  nv.resize(3);
  const casacore::MVDirection tangent(convertQuantity("12h30m00.000","rad"),
                                  convertQuantity("-45.00.00.000","rad"));
  const casacore::MDirection tangentDir(tangent, casacore::MDirection::J2000);
  const casacore::Vector<casacore::RigidVector<casacore::Double, 3> >& uvw = acc.rotatedUVW(tangentDir);
  //const casacore::Vector<casacore::RigidVector<casacore::Double, 3> >& uvw = acc.uvw();
  
  casacore::Matrix<double> normalMatr(3,3,0.);
  const casacore::uInt nRow = acc.nRow();
  for (casacore::uInt row = 0; row<nRow; ++row) {
       if ((int(acc.feed1()[row]) != beam) && (beam>=0)) {
           continue;
       }
       normalMatr(0,0) += casacore::square(uvw[row](0));
       normalMatr(1,1) += casacore::square(uvw[row](1));
       normalMatr(2,2) += casacore::square(uvw[row](2));
...
void doReadOnlyTest(const IConstDataSource &ds) {
  ...
  for (IConstDataSharedIter it=ds.createConstIterator(sel,conv);it!=it.end();++it) {  
       ...
       for (int beam=0;beam<5;++beam) {
            ...
            std::cout<<"angle="<<acos(dotproduct)/casacore::C::pi*180.<<std::endl;
       }
       /*
       ...
       */
       ...
       /*
       const casacore::Vector<casacore::RigidVector<casacore::Double, 3> >& uvw = it->rotatedUVW(tangentDir);
       const casacore::Vector<casacore::uInt>& beamIDs = it->feed1();
...
void makeRaster() {
  // Virgo
  casacore::MVDirection tangent(convertQuantity("12h30m49.43","rad"),convertQuantity("12.23.29.1","rad"));
  // 0407-658
  //casacore::MVDirection tangent(convertQuantity("04h08m20.38","rad"),convertQuantity("-65.45.09.1","rad"));
  ...
  size_t counter = 0;
  const double resolutionInRad = resolution / 180. * casacore::C::pi;
  for (int x = -halfNOffsets; x <= halfNOffsets; ++x) {
       ...
       for (int y = -halfNOffsets; y <= halfNOffsets; ++y) {
            ...
            casacore::MVDirection testDir(tangent);
            testDir.shift(xOffset,yOffset, casacore::True);
            ...
            os<<testDir.getLong()/casacore::C::pi*180.<<" "<<testDir.getLat()/casacore::C::pi*180.<<" "<<counter<<" "<<x<<" "<<y * dir<<" "<<dirStr<<std::endl;
       }
  }
}
...
void doTest() {
  // 1549-790
  const casacore::MVDirection tangent(convertQuantity("15h56m58.871","rad"), convertQuantity("-79.14.04.28","rad"));
  // 1934-638
  //const casacore::MVDirection tangent(convertQuantity("19h39m25.03","rad"),convertQuantity("-63.42.45.6","rad"));
  //const casacore::MDirection tangentDir(tangent, casacore::MDirection::J2000);
  
  // 1610-771
  const casacore::MVDirection dir(convertQuantity("16h17m49.278","rad"), convertQuantity("-77.17.18.46","rad"));

  // 1936-623
  //const casacore::MVDirection dir(convertQuantity("19h41m21.77","rad"),convertQuantity("-62.11.21.06","rad"));
  
  // 1547-795
  //const casacore::MVDirection dir(convertQuantity("15h55m21.65","rad"), convertQuantity("-79.40.36.3","rad"));
  

  //casacore::MVDirection testDir = tangent;
  // Virgo
  //casacore::MVDirection testDir(convertQuantity("12h30m49.43","rad"),convertQuantity("12.23.28.01","rad"));
  // 1934-638
  //casacore::MVDirection testDir(convertQuantity("19h39m25.03","rad"),convertQuantity("-63.42.45.6","rad"));

  //casacore::MVDirection testDir(convertQuantity("07h12m43.98","rad"),convertQuantity("18.57.52.4","rad"));
  //casacore::MVDirection testDir(convertQuantity("15h39m16.97","rad"),convertQuantity("-78.42.06.17","rad"));
  ...
  casacore::MPosition mroPos(casacore::MVPosition(casacore::Quantity(370.81, "m"), casacore::Quantity(-26.6991531922, "deg"), 
           casacore::Quantity(116.6310372795, "deg")),casacore::MPosition::Ref(casacore::MPosition::WGS84));
  casacore::Quantity q;
  ASKAPCHECK(casacore::MVTime::read(q, "today"), "MVTime::read failed");
  std::cout<<"Current UT MJD: "<<q<<std::endl;
  casacore::MEpoch when(casacore::MVEpoch(q), casacore::MEpoch::Ref(casacore::MEpoch::UTC));
  casacore::MeasFrame frame(mroPos, when);
  casacore::MVDirection testDir = casacore::MDirection::Convert(casacore::MDirection(casacore::MDirection::SUN), casacore::MDirection::Ref(casacore::MDirection::J2000,frame))().getValue();
 
   */
  // put Sun position here for beamforming
  casacore::MVDirection testDir(convertQuantity("20h54m47.18","rad"),convertQuantity("-17.24.01.5","rad"));
  ...
  offset1 = ofq / 180. * casacore::C::pi;
  offset2 = -0.5 / 180. * casacore::C::pi;
  */
  
  std::cout<<"separation (dir vs. tangent): "<<dir.separation(tangent)*180./casacore::C::pi<<" deg, offsets (deg): "
     <<offset1*180./casacore::C::pi<<" "<<offset2*180./casacore::C::pi<<std::endl;
  
  const casacore::MVDirection backupTestDir(testDir);
  ...
  */
  ...
  const double offset1inDeg = offset1 / casacore::C::pi * 180.;
  const double offset2inDeg = offset2 / casacore::C::pi * 180.;
  ...
  for (size_t i=0; i<xOffsets.size(); ++i) {
      offset1 = xOffsets[i] / 180. * casacore::C::pi;
      offset2 = yOffsets[i] / 180. * casacore::C::pi;
      testDir = backupTestDir;
      testDir.shift(offset1*factor,offset2*factor, casacore::True);
      std::cout<<"offset ("<<xOffsets[i]<<","<<yOffsets[i]<<") applied to test direction: "<<printDirection(testDir)<<std::endl;
      //std::cout<<360+testDir.getLong()/casacore::C::pi*180.<<" "<<testDir.getLat()/casacore::C::pi*180.<<std::endl;
  }
